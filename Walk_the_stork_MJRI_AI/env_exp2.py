...
class WalkTheStork(gym.Env):
    ...
    def get_stacked_buffer(self):
        ...
        frames[:50] = 255
        threshold_value = 60
        frames_black = (np.where(frames < threshold_value, frames, 0) * 255).astype(
            np.uint8
        )
    ...
    def step(self, action):
        ...
        if self._is_dead(obs_next):
            ...
            final_distance = self._read_distance_ocr(frame_org[:250])
            if final_distance is not None:
                self.episode_distance = final_distance
                # print(f"⚰️ 최종 거리: {final_distance}m")

            self.current_state = DIE
            reward = -100.0 + self.episode_distance  # 죽음 패널티 + 진행 거리 보상
            terminated = True
            ...
            # 각 열에서 첫 번째로 나타나는 검은 픽셀의 y 좌표 중 최솟값
            # 146은 꼬리
            mask = obs_next[..., -1] > 0

            # 각 열(x)에서 True가 하나라도 있는지 확인
            column_mask = mask.any(axis=0)  # (width,) 형태

            if column_mask.any():
                # 첫 번째로 1이 등장하는 x 위치
                first_x = np.argmax(column_mask)
                # 마지막으로 1이 등장하는 x 위치
                last_x = len(column_mask) - 1 - np.argmax(column_mask[::-1])

            if first_x > self.first_x_threshold and last_x < self.last_x_threshold:
                # good position
                reward = good_position_reward
            else:
                # bad position - threshold에서 멀어질수록 큰 패널티
                if first_x <= self.first_x_threshold:
                    distance = self.first_x_threshold - first_x
                    reward = -distance_penalty * distance  # 거리에 비례하는 패널티
                elif last_x >= self.last_x_threshold:
                    distance = last_x - self.last_x_threshold
                    reward = -distance_penalty * distance  # 거리에 비례하는 패널티
